...
@load "Density_distributions\\$(pathname_density_distrib[1:end-3])_N_$(N).jld2" CS_densities_cuts_norm # Load the density (in atom/λ^3)
...
for (i, Ω) in ProgressBar(enumerate(Ω_RO))
    tasks = map(chunks_n) do chunk # Split the different density distributions into chuncks solved on each core
        Threads.@spawn solve_random_distrib_vary_Ω(chunk, fsolve_diff_eq, op_list, N, n_list, window_t, window_var, threshold_box, t_end, save_sol, Ω)
    end
    sol_tasks = fetch.(tasks)
    @save "solutions/Sat_$(minimum(sat))to$(maximum(sat))_n0_$(round(mean(n_list), digits=2))_N_$N/sol_N_$(N)_Sat_$(sat[i])_stripe_distribution_$(pathname_density_distrib[1:end-3])_n0_$(round(mean(n_list), digits=2)).jld2" N n_list sat sol_tasks
    sol_tasks = nothing
end