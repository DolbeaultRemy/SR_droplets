...
function solve_random_distrib_vary_Ω(chunk, functs, op_list, N, n, d0_lb, window_t, window_var, threshold_box, f_counter, save_sol, Ω_RO)
    ...
    for i in chunk
        ...
        @save "r0/r0_N_$(N)_n0_$(n)_r_$i.jdl2" r0 L
        ...
        if ~SciMLBase.successful_retcode(sol) | (sol.t[end] == t_end) | ((popup_t[end][end] < 0) | (maximum(popup_t[end]) > N)) # If solution does not CV, add it to the error
            push!(nbr_error_t, i)
        end
    end
