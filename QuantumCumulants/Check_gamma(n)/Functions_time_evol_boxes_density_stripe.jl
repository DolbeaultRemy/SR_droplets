...
function create_random_distribution(N, n, i, Ω_RO)
    # Compute distribution
    L = (N/n)^(1/3) # Change the volume to keep the 3D density = n and the atom nbr = N
    r0 = [[rand(Float64)*L, rand(Float64)*L, rand(Float64)*L] for atm in 1:N]

    # Save the atoms position for comparison with QuantumOptics
    
    @save "r0/Sat_$(minimum(sat))to$(maximum(sat))_n0_$(round(mean(n_list), digits=2))_N_$N/r0_N_$(N)_density_idx_$(i)_sat_$(round(2*Ω_RO^2, digits=1))_n0_$(round(mean(n_list), digits=2)).jdl2" r0 L
...
function solve_random_distrib_vary_Ω(chunk, fsolve_diff_eq, op_list, N, n_list, window_t, window_var, threshold_box, t_end, save_sol, Ω_RO)
    ...
    for i in chunk
        ...
        try
            while !CV_sim
                p0 = create_random_distribution(N, n_list[i], i, Ω_RO) # Parameters of the random distribution of density n_list[i]
    ...
    return list_t, popup_t, sol_t, nbr_error
end
...
function Create_dir(sat, n_list)
    if !isdir("r0")
        mkdir("r0")
    end
    ...
    if !isdir("solutions/Sat_$(minimum(sat))to$(maximum(sat))_n0_$(round(mean(n_list), digits=2))_N_$N")
        mkdir("solutions/Sat_$(minimum(sat))to$(maximum(sat))_n0_$(round(mean(n_list), digits=2))_N_$N")
    end
    if !isdir("r0/Sat_$(minimum(sat))to$(maximum(sat))_n0_$(round(mean(n_list), digits=2))_N_$N")
        mkdir("r0/Sat_$(minimum(sat))to$(maximum(sat))_n0_$(round(mean(n_list), digits=2))_N_$N")
    end
end