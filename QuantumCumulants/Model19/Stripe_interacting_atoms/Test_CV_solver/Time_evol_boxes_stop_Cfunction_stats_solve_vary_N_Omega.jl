...
r = 100
sat = [35;]
n0 = 1000 # Atoms/λ^-3 (density already integrated along z)
...
for (i, N) in ProgressBar(enumerate(N_list))
    ...
    for (j, Ω) in ProgressBar(enumerate(Ω_RO))
        tasks = map(chunks) do chunk # Split the different distributions into chuncks solved on each core
            Threads.@spawn solve_random_distrib_vary_Ω(chunk, functs, op_list, N, n0, kl, d0_lb, window_t, window_var, threshold_box, i, false, Ω)
        end
