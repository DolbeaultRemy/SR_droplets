...
function solve_random_distrib_vary_Ω(chunk, functs, op_list, N, n, kl, d0_lb, window_t, window_var, threshold_box, f_counter, save_sol, Ω_RO)
    ...
    for i in chunk
        ...
        Ωij_ = [Ω_CS[i, j] for i = 1:N for j=1:N if i≠j] #* 0 # TEST WITHOUT Hspinflip
        ...
        sol = OrdinaryDiffEq.solve(prob, OrdinaryDiffEq.DP5());
                    # callback = stop_cb) #dtmin = 1e-3 dtmin = 1e-2)
        ...
        if ~SciMLBase.successful_retcode(sol) | ((popup_t[end][end] < 0) | (maximum(popup_t[end]) > N)) #  (sol.t[end] == t_end)  # If solution does not CV, add it to the error
            push!(nbr_error_t, i)
        end
    end
